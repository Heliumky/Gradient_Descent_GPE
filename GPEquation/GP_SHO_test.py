...
def gradient_descent2 (H0, psi, g, step_size, steps, maxdim=100000000, cutoff=1e-16):
...
def gradient_descent3 (H0, psi, g, step_size, steps, maxdim=100000000, cutoff=1e-16):
    ...
    for n in range(steps):
        # Gradient descent
        psi, en, ts = gdGP.gradient_descent_GP_MPS (steps, psi, H0, g, step_size, niter=5, maxdim=maxdim, cutoff=cutoff, linesearch=True)
        #en *= dx
        enss.append(en)
        print('GD3',n,en)
        t2 = time.time()
...
if __name__ == '__main__':
    ...
    psi, ens, terrs = dmrg.dmrg (1, psi, H0, [maxdim]*10, cutoff=1e-12, krylovDim=10, verbose=True)

    # TDVP
    dt = dx**2
    print('dt',dt)
    psi_TDVP, ens_TDVP, ts0 = imag_time_evol (H0, psi, g, dt, steps=100, maxdim=maxdim, cutoff=cutoff, krylovDim=krylovDim)
    #psi_TDVP[0] /= np.sqrt(dx**2)
    #with open('7_vortex.pkl', 'wb') as f:
    #    pickle.dump(psi_TDVP, f)

    # Gradient descent
    step_size = dt*0.01
    psi_GD, ens_GD, ts1 = gradient_descent (H0, psi, g, step_size, steps=200, maxdim=maxdim, cutoff=cutoff)

    # Gradient descent
    psi_GD2, ens_GD2, ts2 = gradient_descent2 (H0, psi, g, step_size, steps=100, maxdim=maxdim, cutoff=cutoff)

    # Gradient descent
    psi_GD3, ens_GD3, ts3 = gradient_descent3 (H0, psi, g, step_size, steps=10, maxdim=maxdim, cutoff=cutoff)
    ...
    ax2.plot(ts1, abs(ens_GD-en), marker='x', label='GD')
    ax2.plot(ts2, abs(ens_GD2-en), marker='+', label='GD2')
    ax2.plot(ts3, abs(ens_GD3-en), marker='*', label='GD3')
    ax2.set_yscale('log')
    ax2.legend()
    plt.show()
    ...
    psi2 = psi_sqr (psi)
    X, Y, Z = pltut.plot_2D (psi2, x1, x2, ax=ax, func=absSqr, label='Init')
    ...
    psi_GD = qtt.normalize_MPS_by_integral (psi_GD, x1, x2, Dim=2)
    X_GD, Y_GD, Z_GD2 = pltut.plot_2D_proj (psi_GD, x1, x2, ax=ax, func=absSqr, label='GD')
    fig.savefig('GD_proj.pdf')
    ax.legend()

    fig, ax = plt.subplots(figsize=(10, 8))
    psi_GD = qtt.normalize_MPS_by_integral (psi_GD, x1, x2, Dim=2)
    X_TDVP, Y_TDVP, Z_TDVP = pltut.plot_2D_proj (psi_TDVP, x1, x2, ax=ax, func=absSqr, label='GD')
    fig.savefig('TDVP_proj.pdf')
    ax.legend()
